...
public class Game {
    ...
    public Bowl[] bowls = new Bowl[7];
    public Bowl[] bowls2 = new Bowl[7];
    ...
    public int getTotalSeedsInGame(){
        int sum = 0;
        for (Bowl b : bowls){
            sum += b.getSeeds();
        }
    ...
    public void prettyPrintGameConfiguration(){
    ...
    public String getStringConfigurationOfCurrentGame(){
        String config = "";
    ...
    public void init() {
        ...
        bowls[0].setNextBowl(bowls2[1]);
        bowls2[0].setNextBowl(bowls[1]);
    ...
    private boolean incrementBowl(Bowl b, int increments, int initiatingPlayer){
        ...
        return incrementBowl(b.getNextBowl(), increments, initiatingPlayer);  //recursively move
    ...
    public boolean startFromBowlP1(int bowlNumber){
        ...
        boolean lastMoveInInitiatingPlayerTray = incrementBowl(currentBowl.getNextBowl(), increments, 1);
        return lastMoveInInitiatingPlayerTray; // Play or Not to Play Again?
    ...
    // to start the moves. The number of moves depends of the seeds in the current bowl.
    // The bowlNumber can be from 0 to 6
    public boolean startFromBowlP2(int bowlNumber){
        Bowl currentBowl = bowls2[bowlNumber];
        int increments = currentBowl.getSeeds();
        if (increments < 1) return false;
        currentBowl.setSeeds(0);
        boolean lastMoveInInitiatingPlayerTray = incrementBowl(currentBowl.getNextBowl(), increments, 2);
        return lastMoveInInitiatingPlayerTray; //Play or Not to Play Again?
    ...
    public int getPlayer1Score(){
        return bowls[0].getSeeds();
    }

    public int getPlayer2Score(){
        return bowls2[0].getSeeds();
    }

    public int checkWining(){
        if (check1Empty() || check2Empty()) {
    ...
    private int checkWinner() {
        ...
        if (bowls[0].getSeeds() > bowls2[0].getSeeds()) return 1;
        else if (bowls2[0].getSeeds() > bowls[0].getSeeds()) return 2;
        else if (bowls[0].getSeeds() == bowls2[0].getSeeds()) return -1;
        return 0;
    }


    // Check if all the bowls belonging to the Player 1 are empty or not?
    public Boolean check1Empty(){
